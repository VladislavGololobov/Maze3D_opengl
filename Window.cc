...
void Window::create_window() {
	_window = std::shared_ptr<SDL_Window>(
			SDL_CreateWindow("Maze3D_opengl", SDL_WINDOWPOS_CENTERED,
...
void Window::set_perspective_mode() {
...
Window::Window(int w, int h): _width(w), _height(h) {
	...
	_road_tex = std::make_shared<texture>("road.jpg");
...
void Window::draw_hall(int my, int mx) {
	...
	glBegin(GL_QUADS);
	glColor3d(1.0, 1.0, 1.0);
//	glColor3d(0.027451, 0.2941, 0.086275);
